...
const Navbar = () => {
  ...
  return (
    <nav className='sticky top-0 border-b border-white border-opacity-20'>
      <div className='max-w-screen-2xl flex items-center justify-between mx-auto px-12 py-7 lg:px-32 lg:pt-16 lg:pb-6'>
        ...
        {/* NavLinks */}
        <div className='items-center hidden gap-28 lg:flex '>
...

        {
          // Mobile Menu
          isOpen ? (
            <div className='lg:hidden fixed w-full z-20 top-0 left-0 bg-purple p-12 rounded-lg border border-white border-opacity-5 backdrop-blur-3xl'>
              <button
                onClick={closeMenu}
                aria-controls='menu'
                aria-expanded='true'
              ...
              <ul id='menu' className='space-y-5 mt-28 mb-36'>
                {[
                  { text: 'Overview', url: 'overview' },
                  { text: 'Timeline', url: 'timeline' },
                  { text: 'FAQs', url: 'faq' },
                  ...
                  <li key={link.url}>
                    <button
                      onClick={closeMenu}
                      aria-label={`Click to move to ${link.text} section`}
                    >
                      <a
                        href={`#${link.url}`}
                        className='font-inter text-lg font-medium tracking-[-1px] text-white'
                      >
                        {link.text}
                      </a>
                    </button>
